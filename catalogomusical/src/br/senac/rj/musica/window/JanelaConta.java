...
public class JanelaConta {
	public static JFrame criarJanelaConta() {
		...
		janelaConta.setResizable(false);  //Janela não poderá ter o tamanho ajustado
		janelaConta.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		janelaConta.setSize(400,300);  //Define o tamnho da janela
		...
		JLabel labelAgencia = new JLabel ("Agencia: ");
		...
		labelNumero.setBounds(50,80,120,20); //Coluna,linha,largura,tamanho
		...
		//Adicionoa os rótulos e os input box na janela
		...
		//Define botões e a localizção deles na janela
		...
		//Define objeto conta para pesqusiar no banco de dados
		...
		botaoConsultar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e){
				try {
					...
					JOptionPane.showMessageDialog(janelaConta, "Preencha os campos agência e número da conta corretamente!!");
				}
		...
		botaoGravar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (resposta == JOptionPane.YES_OPTION) {
					...
					if (titular.length() == 0) {
						JOptionPane.showConfirmDialog(janelaConta, "Preensha o campo titular");
						jTextTitular.requestFocus();
					} else {
						if (!conta.consultarConta(agencia,numero)) {
							if (!conta.cadastrarConta(agencia, numero, titular))
								JOptionPane.showConfirmDialog(janelaConta, "Erro na inclusão do titular");
							else
								JOptionPane.showConfirmDialog(janelaConta, "Inclusão realizada");
						} else { 
							if(!conta.atualizarConta(agencia, numero, titular))
								JOptionPane.showConfirmDialog(janelaConta, "Erro na atualização do titular");
							else
								JOptionPane.showConfirmDialog(janelaConta, "Alteração realizada");
							}
						}
					}
				}
